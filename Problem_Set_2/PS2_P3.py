...
w,v = np.linalg.eig(N)# w = Lambda, v = v
...
plt.clf()
plt.title("<ddT>")
plt.imshow(ddTavg,interpolation='nearest',vmin=0,vmax=5)
...
plt.clf()
plt.title("N")
plt.imshow(N,interpolation='nearest',vmin=0,vmax=5)
plt.colorbar()
plt.savefig("PS2_P3_N_matrix.png")

"""
The <ddT> and N matrices have some similarities (see figures), but they don't
seem to get a whole lot closer with more iterations, so I'm not sure they're
really converging
